 ...
 * @param {Map<String, Set<String>} userSocketsMap - userId를 키, 해당 유저의 클라이언트 소켓 id Set을 값으로 하는 Map instance
 */
const initializeSocket = (io, middlewares, roomManager, userSocketsMap) => {
    ...
    : userSocketsMap.set(socket.user.id, new Set([socket.id]));
		// 소켓 이벤트 핸들러 부착

    socket.on('disconnecting', () => {
      userSocketsMap.get(socket.user.id).delete(socket.id);
    });
